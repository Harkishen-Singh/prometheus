...
queue_config:
  ...
  retry_policy:
    # If set, any samples batch that has the minimum timestamp earlier than
    # min_samples_age given will not be sent to the remote storage.
    # This means that if the batch contains some samples greater than
    # the min_samples_age, they will be rejected as well.
